...
/**
 * Created by ltaoj on 2018/3/3 17:31.
 */
...
public class CircleDrawable extends Drawable {
    ...
    private boolean[] mCornersCircle = new boolean[] {true, true, true, true};
    ...
    public static Bitmap drawableToBitmap(Drawable drawable) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.w(TAG, "drawableToBitmap: Failed to create bitmap from drawable!");
            return null;
        }
        return bitmap;
    }
    ...
    private void updateShaderMatrix() {
        ...
        switch (mScaleType) {
            ...
            case CENTER_CROP:
                ...
                dx = 0;
                dy = 0;
                if (mBitmapWidth * mBorderRect.height() > mBorderRect.width() * mBitmapHeight) { // 宽 > 高
                    scale = mBorderRect.height() / (float) mBitmapHeight;
                    dx = (int) ((mBitmapRect.width() - mBitmapWidth * scale) * 0.5f);
                } else {
                    scale = mBorderRect.width() / (float) mBitmapWidth;
                    dy = (int) ((mBorderRect.height() - mBitmapHeight * scale) * 0.5f);
                }
    ...
    private void redrawBitmapForSquareCorners(Canvas canvas) {
        ...
        if (!mCornersCircle[Corner.TOP_LEFT]) {
            mSquareCornersRect.set(left, top, left + radius, top + radius);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersCircle[Corner.TOP_RIGHT]) {
            mSquareCornersRect.set(right - radius, top, right, top + radius);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersCircle[Corner.BOTTOM_RIGHT]) {
            mSquareCornersRect.set(right - radius, bottom - radius, right, bottom);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersCircle[Corner.BOTTOM_LEFT]) {
    ...
    public float getCornerRadius(@Corner int corner) {
        return mCornersCircle[corner] ? mCornerRadius : 0f;
    }
     ...
     * @return the {@link CircleDrawable} for chaining.
     */
     ...
     * @return the {@link CircleDrawable} for chaining.
     */
    public CircleDrawable setCornerRadius(@Corner int corner, float radius) {
        if (radius != 0 && mCornerRadius != 0 && mCornerRadius != radius) {
            throw new IllegalArgumentException("Multiple nonzero corner radii not yet supported.");
        }

        if (radius == 0) {
            if (only(corner, mCornersCircle)) {
                mCornerRadius = 0;
            }
            mCornersCircle[corner] = false;
        } else {
            if (mCornerRadius == 0) {
                mCornerRadius = radius;
            }
            mCornersCircle[corner] = true;
        }
     ...
     * @return the {@link CircleDrawable} for chaining.
     */
    public CircleDrawable setCornerRadius(float topLeft, float topRight, float bottomRight,
                                          float bottomLeft) {
        ...
        mCornersCircle[Corner.TOP_LEFT] = topLeft > 0;
        mCornersCircle[Corner.TOP_RIGHT] = topRight > 0;
        mCornersCircle[Corner.BOTTOM_RIGHT] = bottomRight > 0;
        mCornersCircle[Corner.BOTTOM_LEFT] = bottomLeft > 0;
        return this;
    }
    ...
    public CircleDrawable setBorderWidth(float width) {
        mBorderWidth = width;
        mBorderPaint.setStrokeWidth(mBorderWidth);
        return this;
    }
    ...
    public CircleDrawable setBorderColor(@ColorInt int color) {
    ...
    public CircleDrawable setBorderColor(ColorStateList colors) {
    ...
    public CircleDrawable setTileModeX(Shader.TileMode tileModeX) {
    ...
    public CircleDrawable setTileModeY(Shader.TileMode tileModeY) {
